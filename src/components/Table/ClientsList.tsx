...
import { formatName, formatDay } from "../../../utils/format";
...
export default function ClientsList({ userDetail }: { userDetail: iUser }) {
  ...
  const { data, refetch, status, isPending } = useQuery({
    ...
    queryFn: () =>
      axios
        .get(
          `https://lubrytics.com:8443/nadh-api-crm/api/clients?perPage=${pageSize}&page=${currentPage}&lead_consultant=${userDetail?.id}`,
          {
            headers: {
        ...
        .then((res) => {
          ...
          return res.data.data.map((client: iClient) => ({
            ...
            updated_on: formatDay(client.meta?.lastUpdated?.time),
